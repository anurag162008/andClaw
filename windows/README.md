...
1. Run **Install Runtime** from Setup tab (auto-provisions Node.js, OpenClaw and Playwright Chromium in WSL).
2. Run **Runtime Doctor** and verify all checks are OK.
3. In Onboarding, use **Complete OAuth** to validate token against OpenRouter API, then press **Refresh Models**.
4. Start gateway from Dashboard.
5. In Settings, save `Auto start on launch` to sync with Windows Startup (HKCU Run).
6. Generate bug report and export transfer from Settings.
