#!/usr/bin/env python3
"""Windows-friendly AndClaw .transfer file decrypter.

Decrypts AndClaw .transfer files that use the same format as
app/src/main/java/com/coderred/andclaw/data/transfer/TransferCrypto.kt.
"""
...
def _safe_extract_zip_bytes(zip_bytes: bytes, target_dir: pathlib.Path) -> None:
    ...
    # Use in-memory read of decrypted bytes and explicit path traversal guard.
...
def _write_output(
    ...
    if output_format == OUTPUT_FORMAT_ZIP:
        target = output_dir / f"{source_transfer.stem}.zip"
        target.write_bytes(decrypted_bytes)
        return

    if output_format == OUTPUT_FORMAT_RAW:
        target = output_dir / source_transfer.stem
        target.write_bytes(decrypted_bytes)
        return

    if output_format == OUTPUT_FORMAT_EXTRACT:
        folder = output_dir / source_transfer.stem
        _safe_extract_zip_bytes(decrypted_bytes, folder)
        return
...
class DecrypterApp:
    def __init__(self, root: Tk) -> None:
        self.root = root
        self.root.title("AndClaw .transfer Decrypter")
        ...
        ttk.Entry(frame, textvariable=self.input_dir, width=70).grid(row=1, column=0, sticky="ew", padx=(0, 8))
        ...
        ttk.Entry(frame, textvariable=self.output_dir, width=70).grid(row=3, column=0, sticky="ew", padx=(0, 8))
        ttk.Button(frame, text="Browse", command=self.browse_output).grid(row=3, column=1, sticky="ew")

        ttk.Label(frame, text="Password").grid(row=4, column=0, sticky="w", pady=(12, 0))
        ttk.Entry(frame, textvariable=self.password, show="*", width=70).grid(row=5, column=0, sticky="ew", padx=(0, 8))

        ttk.Label(frame, text="Output format").grid(row=6, column=0, sticky="w", pady=(12, 0))
        ...
        format_box.grid(row=7, column=0, sticky="w")

        ttk.Label(
            frame,
            text="zip = .zip file, extract = folder, raw = decrypted binary",
        ).grid(row=8, column=0, sticky="w", pady=(6, 0))

        ttk.Button(frame, text="Decrypt All", command=self.run_decrypt).grid(row=9, column=0, sticky="ew", pady=(20, 0))
    ...
    def run_decrypt(self) -> None:
        try:
            if not self.input_dir.get().strip():
                raise ValueError("Input folder select karo")
            ...
            success, failures = decrypt_folder(
                pathlib.Path(self.input_dir.get().strip()),
                pathlib.Path(self.output_dir.get().strip()),
                self.password.get(),
            ...
            if failures:
                ...
                messagebox.showwarning(
                    ...
                    f"{success} file(s) decrypt hui.\n{len(failures)} file(s) fail hui.\n\n{details}",
                )
            else:
...
def _cli(argv: list[str]) -> int:
    if len(argv) in {5, 6} and argv[1] == "--cli":
        ...
        success, failures = decrypt_folder(input_dir, output_dir, password, output_format)
